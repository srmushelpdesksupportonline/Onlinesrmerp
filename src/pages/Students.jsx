...
const STORAGE_KEY = 'studentPageColumns_v2';

const DEFAULT_COLUMNS = [
  'application_no','enrollment_no','full_name','program_name',
  'specialization','personal_email','official_email','mobile',
  'login_password','email_activation_status','student_status',
  'scholarship_type','willing_to_pay','payment_method_chosen',
];

const SCHEMA_COLUMNS = [
  { field: 'application_no',           label: 'Application No',           group: 'ERP', pinned: 'left' },
  { field: 'enrollment_no',            label: 'Enrollment No',            group: 'ERP', pinned: 'left' },
  { field: 'full_name',                label: 'Full Name',                group: 'ERP', pinned: 'left' },
...
const EXCEL_FIELDS = [
  ...
  { field: '2.-Name of Organization',             label: 'Work Exp 2 — Organisation',    group: 'Work Experience' },
  { field: '2.-Designation',                       label: 'Work Exp 2 — Designation',     group: 'Work Experience' },
  { field: '2.-Sector',                            label: 'Work Exp 2 — Sector',          group: 'Work Experience' },
  { field: '2.-Annual Salary Package (In Lacs)',   label: 'Work Exp 2 — Salary (Lacs)',   group: 'Work Experience' },
  { field: '2.-From',                              label: 'Work Exp 2 — From',            group: 'Work Experience' },
  { field: '2.-To',                                label: 'Work Exp 2 — To',              group: 'Work Experience' },
  { field: '2.-Total Experience(In Months)',       label: 'Work Exp 2 — Duration (Months)', group: 'Work Experience' },
...
function CustomisePanel({ visible, onApply, onClose }) {
  ...
  const PINNED_FIELDS = ['application_no','enrollment_no','full_name'];
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {Object.entries(groups).map(([group, cols]) => (
                <div key={group}>
                  ...
                  {cols.map(col => {
                    const isPinned = PINNED_FIELDS.includes(col.field);
                    return (
                      <div key={col.field} onClick={() => toggle(col.field)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px', cursor: isPinned ? 'default' : 'pointer', borderBottom: '1px solid #F9FAFB' }}>
                        <input type="checkbox" checked={selected.includes(col.field)} onChange={() => toggle(col.field)} disabled={isPinned} style={{ accentColor: '#3d4f12', width: 14, height: 14 }} />
                        <span style={{ fontSize: 13, color: isPinned ? '#9CA3AF' : '#374151' }}>
                          {col.label}{isPinned && <span style={{ fontSize: 10, marginLeft: 6, color: '#9CA3AF' }}>(fixed)</span>}
                        </span>
                      </div>
                    );
                  })}
          ...
          <div style={{ width: 195, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {selected.map(field => {
                ...
                const isPinned = PINNED_FIELDS.includes(field);
                return (
                  <div key={field} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderBottom: '1px solid #F9FAFB', fontSize: 13 }}>
                    ...
                    {!isPinned && <button onClick={() => toggle(field)} style={{ background: 'none', border: 'none', color: '#9CA3AF', cursor: 'pointer', fontSize: 15 }}>✕</button>}
                  </div>
                );
              })}
            </div>
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const record = {
          ...
          application_no: String(row['Application No'] || row['application_no'] || '').trim(),
          full_name: String(row['Full Name'] || row['full_name'] || '').trim(),
          program_name: String(row['Program'] || row['program_name'] || '').trim(),
          ...
          academic_year: String(row['Batch/Year'] || row['academic_year'] || '').trim(),
        ...
        const { error: e } = await supabase.from('student_master').upsert(record, { onConflict: 'enrollment_no' });
        ...
        {!results ? (
          <>
            <div style={{ background: '#F0F9FF', border: '1px solid #BAE6FD', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 12 }}>
              <strong>Expected columns:</strong> Application No, Enrollment No, Full Name, Program, Specialization, Personal Email, Official Email, Mobile, Current Semester, Batch/Year
            </div>
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
              <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 8, padding: 14, textAlign: 'center' }}><div style={{ fontSize: 28, fontWeight: 700, color: '#166534' }}>{results.success}</div><div style={{ fontSize: 12, color: '#166534' }}>Uploaded</div></div>
              <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: 14, textAlign: 'center' }}><div style={{ fontSize: 28, fontWeight: 700, color: '#DC2626' }}>{results.failed}</div><div style={{ fontSize: 12, color: '#DC2626' }}>Failed</div></div>
              <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 8, padding: 14, textAlign: 'center' }}><div style={{ fontSize: 28, fontWeight: 700, color: '#1E40AF' }}>{results.total}</div><div style={{ fontSize: 12, color: '#1E40AF' }}>Total</div></div>
            </div>
            {results.errors.length > 0 && <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: 12, fontSize: 12, maxHeight: 150, overflowY: 'auto', marginBottom: 16 }}>{results.errors.map((e, i) => <div key={i}>{e}</div>)}</div>}
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
...
export default function Students() {
  ...
  const loadStudents = useCallback(async () => {
    ...
    try {
      ...
      if (filterProgram) {
        ...
        const fullName = fullNameMap[filterProgram] || filterProgram;
        query = query.ilike('program_name', `%${fullName.split(' ').pop()}%`);
      }
  ...
  const buildColDef = (field) => {
    ...
    const pinned    = schemaCol?.pinned || undefined;
    ...
    if (isExcel) {
      return { headerName: field, field: '__raw__' + field, minWidth: 120, flex: 1, cellStyle: copyStyle, valueGetter: p => p.data?.raw_data?.[field] ?? '' };
    }

    const specialCols = {
      application_no:          { minWidth: 130, flex: 1, maxWidth: 180, pinned: 'left', cellStyle: copyStyle },
      enrollment_no:           { minWidth: 140, flex: 1, maxWidth: 170, pinned: 'left', cellStyle: { ...copyStyle, fontWeight: 600 } },
      full_name:               { minWidth: 160, flex: 1, maxWidth: 220, pinned: 'left', cellStyle: copyStyle },
      program_name:            { minWidth: 180, flex: 1, maxWidth: 280 },
      specialization:          { minWidth: 160, flex: 1, maxWidth: 240 },
      student_status:          { minWidth: 110, width: 120, cellRenderer: p => <StatusBadge value={p.value} /> },
      email_activation_status: { minWidth: 110, width: 120, cellRenderer: p => <EmailBadge value={p.value} /> },
      official_email:          { minWidth: 220, flex: 2, cellStyle: copyStyle },
      personal_email:          { minWidth: 200, flex: 2, cellStyle: copyStyle },
      mobile:                  { minWidth: 130, width: 145, cellStyle: copyStyle },
      whatsapp_no:             { minWidth: 130, width: 145, cellStyle: copyStyle },
      login_password:          { minWidth: 130, flex: 1, maxWidth: 180, cellStyle: copyStyle },
      current_semester:        { minWidth: 100, width: 110 },
      created_at:              { minWidth: 110, width: 120, valueFormatter: p => p.value ? new Date(p.value).toLocaleDateString('en-IN') : '—' },
      dob:                     { minWidth: 110, width: 120 },
      aadhaar_no:              { minWidth: 140, flex: 1, maxWidth: 180, cellStyle: copyStyle },
      abc_id:                  { minWidth: 140, flex: 1, maxWidth: 180, cellStyle: copyStyle },
      welcome_email_sent:      { minWidth: 130, width: 140, valueFormatter: p => p.value ? 'Yes' : 'No' },
      password_generated:      { minWidth: 130, width: 140, valueFormatter: p => p.value ? 'Yes' : 'No' },
    };

    return { headerName: label, field, minWidth: 100, flex: 1, pinned, cellStyle: copyStyle, ...(specialCols[field] || {}) };
  };
  ...
  // header ~60px, page-header ~64px, filter-row ~60px (if open), bottom-bar ~44px, padding ~24px
  const gridHeight = `calc(100vh - ${60 + 64 + (showFilters ? 68 : 0) + 44 + 24}px)`;
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', background: '#F9FAFB', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* ── Always-visible Filter + Search Row ── */}
      ...
      <div style={{ padding: '0 24px 24px' }}>
        <div style={{ width: '100%', maxHeight: 'calc(100vh - 160px)', overflow: 'hidden' }}>
          <AgGridReact
            ref={gridRef}
            ...
            statusBar={{
              statusPanels: [
                ...
                { statusPanel: 'agTotalRowCountComponent', key: 'totalRowComponent', align: 'center' },
      ...
      <div style={{ padding: '8px 24px', background: '#fff', borderTop: '1px solid #e8ead4', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        <div style={{ fontSize: 13, color: '#6B7280' }}>
          Total Records: <strong style={{ color: '#2d3a0e' }}>{totalCount}</strong>
          {(filterProgram || filterStatus || filterSem || searchText) && (
            <span style={{ marginLeft: 10, color: '#c8a84b', fontWeight: 600 }}>
              · Filtered: {filteredStudents.length}
            </span>
          )}
        </div>
